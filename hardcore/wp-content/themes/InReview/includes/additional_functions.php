		...
		<?php 
			...
			for ( $i=1; $i <= $features_number; $i++ ) { ?>
				<div class="et_setting" style="margin: 13px 0 26px 4px; overflow: hidden;">
					...
					<input type="text" style="width: 15em;" value="<?php if ( isset($features[$i-1]) ) echo esc_attr($features[$i-1]); ?>" name="et_feature_name[]" id="et_feature_name<?php echo $i; ?>" /></p>
...
add_action('save_post', 'inreview_custom_panel_save');
function inreview_custom_panel_save($post_id){
	global $pagenow;
	...
	$temp_array['et_fs_price'] = isset($_POST["et_fs_price"]) ? esc_attr($_POST["et_fs_price"]) : '';
	$temp_array['et_affiliate_link'] = isset($_POST["et_affiliate_link"]) ? esc_attr($_POST["et_affiliate_link"]) : '';
	$temp_array['et_affiliate_text'] = isset($_POST["et_affiliate_text"]) ? esc_attr($_POST["et_affiliate_text"]) : '';
