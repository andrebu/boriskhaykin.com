					...
					<div class='et-protected'>
						<div class='et-protected-form'>
							<form action='<?php echo esc_url( home_url('wp-login.php') ) ?>' method='post'>
								<p><label><?php esc_html_e('Username','Notebook'); ?>: <input type='text' name='log' id='log' value='<?php echo esc_attr($user_login); ?>' size='20' /></label></p>
								<p><label><?php esc_html_e('Password','Notebook'); ?>: <input type='password' name='pwd' id='pwd' size='20' /></label></p>
								<input type='submit' name='submit' value='Login' class='etlogin-button' />
							</form> 
						</div> <!-- .et-protected-form -->
						<p class='et-registration'><?php printf( __( 'Not a member? <a href="%s">Register today!</a>', 'Notebook' ), esc_url( site_url('wp-login.php?action=register', 'login_post') ) ); ?></a></p>
					</div> <!-- .et-protected -->
				</div> <!-- end #et-login -->
